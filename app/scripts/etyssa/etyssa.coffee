...
angular.module('etyssa', ["etyssa.config"])
  ...
  .factory('Categories', ["$resource", "GENERAL_CONFIG", ($resource, GENERAL_CONFIG) ->
    $resource "#{GENERAL_CONFIG.API_HOST}/rest/categories?token=:token"       , {token:GENERAL_CONFIG.ETYSSA_API_TOKEN}, {}
  ])
  .factory('Search',     ["$resource", "GENERAL_CONFIG", ($resource, GENERAL_CONFIG) ->
    $resource "#{GENERAL_CONFIG.API_HOST}/rest/search?cat=:cat&token=:token"  , {token:GENERAL_CONFIG.ETYSSA_API_TOKEN}, {}
  ])
